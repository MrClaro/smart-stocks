...
def predict(symbol):
    ...
    df = pd.read_csv(
        ...
        + str(symbol)
        + "?includeAdjustedClose=true&interval=1d&events=history&period2="
        + str(endepoch)
        + "&period1="
        + str(startepoch)
    )
    ...
    while t < numberOfFuturePrediction:
        X = input_for_future_prediction[
            len(input_for_future_prediction) - time_step :
        ].copy()
    ...
    for i in range(len(df2), len(df2) + numberOfFuturePrediction, 1):
        ...
        df3.loc[count].Percent_Change = (
            difference * 100 / input_for_future_prediction[i - 1]
        )[0]
...
def scrapeit():
    ...
    if request.method == "GET" and t is not None:
        return jsonify({"status": 1, "data": nse(t)})
    else:
        return jsonify({"status": 0, "error": "Method Not Callable"})


@app.route("/prediction")
def mod():
    symbol = request.args.get("symbol")
    if request.method == "GET" and symbol is not None:
        return jsonify({"status": 1, "data": predict(symbol)})
    else:
        return jsonify({"status": 0, "error": "Method Not Callable"})
